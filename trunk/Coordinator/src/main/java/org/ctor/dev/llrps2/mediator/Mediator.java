...
public class Mediator {
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println("started mediator");
        mgr.start();
    }
    ...
    private void scanRound() {
        ...
        for (RoundMessage round : roundMediationManager.getRounds()) {
            if (!round.isAssigned()) {
                ...
                final EnrolledAgent leftAgent = agentEnrollmentManager
                        .findAgent(round.getLeft());
                if (leftAgent == null || leftAgent.connections() <= 0) {
                    LOG.info("fail: no connection for left agent");
                    continue;
                }
                final EnrolledAgent rightAgent = agentEnrollmentManager
                        .findAgent(round.getRight());
                if (rightAgent == null || rightAgent.connections() <= 0) {
                    LOG.info("fail: no connection for right agent");
                    continue;
                }
                final SessionHandler leftSession = leftAgent.pollSession();
