...
public class ResultTest extends AbstractTest {
    ...
    public void testRandom() throws Exception {
        ...
        for (Round round : contest.getRounds()) {
            ...
            assertTrue(result.getLeftGames() + result.getRightGames()
                    + result.getDrawGames() != 0);
            assertTrue(result.getMaxLeftStraightGames()
                    + result.getMaxRightStraightGames() != 0);
        }
        tx.commit();
    }
    ...
    private Contest doContest(TestContest moveCallback, int agentCount,
            int roundCount, int gameCount) {
